...
import { Check, AlertTriangle, Info, Search, Palette, Type, Box } from 'lucide-react';
  ...
  return (
    <div className={`${theme.layout.pageContainer} ${theme.layout.pagePadding} ${theme.layout.sectionSpacing} h-full flex flex-col`}>
      <PageHeader 
        ...
        subtitle="Visual regression suite for core design system atoms." 
        icon={Box}
      />
      
      <div className={theme.layout.panelContainer}>
        <div className={`p-4 ${theme.layout.headerBorder} bg-slate-50/50`}>
             <p className="text-xs font-bold text-slate-500 uppercase tracking-widest">Theme: <span className="text-nexus-600">{theme.mode}</span> • Density: <span className="text-nexus-600">{theme.density}</span></p>
        </div>
