...
export const NotificationCenter: React.FC = () => {
    ...
    // Optimization: Vibration API provides tactile feedback on mobile devices when a new high-priority alert arrives
    useEffect(() => {
    ...
    return (
        <div className="relative group">
            ...
            <div className={`absolute right-0 mt-3 w-96 ${theme.components.card} overflow-hidden z-50 hidden group-hover:block animate-in slide-in-from-top-2 origin-top-right shadow-2xl border ${theme.colors.border}`}>
                <div className={`p-4 border-b ${theme.colors.border} ${theme.colors.surface} flex justify-between items-center bg-slate-50/50`}>
                    <h3 className="font-bold text-slate-900 text-sm uppercase tracking-tighter">System Notifications</h3>
                    <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest">{unreadCount} UNREAD ENTRIES</span>
                </div>
                <div className="max-h-96 overflow-y-auto scrollbar-thin bg-white">
                    {notifications.length > 0 ? notifications.map(n => (
                        <div 
                            ...
                            className={`p-4 border-b ${theme.colors.border.replace('border-', 'border-b-')}50 hover:bg-slate-50 flex gap-4 items-start cursor-pointer transition-colors ${!n.isRead ? `${theme.colors.semantic.info.bg}/30` : ''}`}
                        >
                            ...
                            <div className="flex-1 min-w-0">
                                <div className="flex justify-between items-start gap-2">
                                    <p className="text-sm font-black text-slate-800 leading-tight truncate">{n.title}</p>
                                    <span className="text-[10px] text-slate-400 font-mono whitespace-nowrap">
                                        {isMounted ? getTimeAgo(n.date) : new Date(n.date).toLocaleDateString()}
                                    </span>
                                </div>
                                <p className="text-xs text-slate-500 mt-1 leading-relaxed line-clamp-2 font-medium">{n.message}</p>
                                <div className="flex flex-wrap gap-1.5 mt-2.5">
                                    {n.category && (
                                        <span className="inline-block px-1.5 py-0.5 rounded text-[9px] font-black bg-slate-100 text-slate-500 border border-slate-200 uppercase tracking-tight">
                                            {n.category}
                                        </span>
                                    )}
                                    {n.link && (
                                        <span className="inline-block px-1.5 py-0.5 rounded text-[9px] font-black bg-nexus-50 text-nexus-600 border border-nexus-100 uppercase tracking-tight">
