...
import { History, Clock, X, Terminal, CheckCircle, ShieldAlert, Zap } from 'lucide-react';
...
export const ActivitySidecar: React.FC<ActivitySidecarProps> = ({ isOpen, onClose }) => {
  ...
  return (
    <div className={`fixed inset-y-0 right-0 w-96 ${theme.colors.surface} border-l border-slate-200 shadow-[0_0_40px_rgba(0,0,0,0.15)] z-[100] flex flex-col animate-in slide-in-from-right duration-500`}>
      <div className={`p-6 border-b ${theme.colors.border} flex justify-between items-center bg-slate-50/50`}>
        ...
        <button onClick={onClose} className={`p-2 hover:bg-slate-100 rounded-full text-slate-400 transition-all active:scale-90`}><X size={20} /></button>
      </div>
                ...
                {auditLogs.slice(0, 50).map((log, idx) => (
                    <div key={`${log.date}-${idx}`} className="relative pl-8 border-l-2 border-slate-100 group">
                        ...
                        <div className="flex justify-between items-center mb-1.5">
                            ...
                            <span className={`text-[9px] font-mono font-bold text-slate-400 uppercase`}>{getTimeString(log.date)}</span>
                        </div>
      ...
      <div className={`p-6 border-t ${theme.colors.border} bg-slate-50/50`}>
         <button className={`w-full py-4 bg-slate-900 text-white rounded-2xl text-[10px] font-black uppercase tracking-[0.2em] hover:bg-black transition-all shadow-xl active:scale-95 border border-white/10`}>
