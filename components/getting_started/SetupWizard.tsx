        ...
        <div className={`fixed inset-0 z-[100] ${theme.colors.background} flex flex-col animate-in slide-in-from-bottom-10 duration-300`}>
            <div className={`flex items-center justify-between px-8 py-6 bg-white border-b ${theme.colors.border} shrink-0`}>
                <div className="flex items-center gap-4">
                    <div className="p-3 bg-nexus-600 text-white rounded-xl shadow-lg shadow-nexus-500/30">
                        <Rocket size={24}/>
                    </div>
                    <div>
                        <h1 className="text-xl font-black text-slate-900 tracking-tight">Enterprise Setup</h1>
                        <p className="text-sm text-slate-500 font-medium">Configure your environment in 4 steps.</p>
                    </div>
                </div>
                ...
                <div className="flex flex-col h-full items-center pt-12 pb-24">
                                ...
                                return (
                                    <div key={step.id} className="flex flex-col items-center gap-3">
                                        <div className={`w-12 h-12 rounded-full flex items-center justify-center transition-all duration-300 ${
                                            isActive ? 'bg-nexus-600 text-white shadow-xl scale-110 ring-4 ring-white' :
                                            isComplete ? 'bg-green-500 text-white ring-4 ring-white' :
                    ...
                    <div className="w-full max-w-lg bg-white p-10 rounded-3xl shadow-xl border border-slate-100 animate-in fade-in slide-in-from-bottom-4 duration-500">
                        ...
                        {currentStep === 2 && (
                            <div className="space-y-6">
                                <div className="text-center mb-8">
                                    <h3 className="text-2xl font-black text-slate-900 mb-2">Define Portfolio Structure</h3>
                                    <p className="text-sm text-slate-500">Create your top-level Enterprise Project Structure (EPS).</p>
                                </div>
                                <div>
                                    <label className="block text-sm font-bold text-slate-700 mb-2">Portfolio Name</label>
                                    <Input value={epsName} onChange={e => setEpsName(e.target.value)} placeholder="e.g. Global Infrastructure" className="h-12 text-lg" />
                                </div>
                            </div>
                        )}
                        ...
                        {currentStep === 3 && (
                            <div className="space-y-6">
                                <div className="text-center mb-8">
                                    <h3 className="text-2xl font-black text-slate-900 mb-2">Strategic Program</h3>
                                    <p className="text-sm text-slate-500">Group related projects under a unifying program.</p>
                                </div>
                                <div>
                                    <label className="block text-sm font-bold text-slate-700 mb-2">Program Name</label>
                                    <Input value={programName} onChange={e => setProgramName(e.target.value)} placeholder="e.g. Digital Transformation" className="h-12 text-lg" />
                                </div>
                            </div>
                        )}
                        ...
                        {currentStep === 4 && (
                            <div className="space-y-6">
                                <div className="text-center mb-8">
                                    <h3 className="text-2xl font-black text-slate-900 mb-2">Launch Pilot Project</h3>
                                    <p className="text-sm text-slate-500">Create your first active project to start tracking.</p>
                                </div>
                                <div>
                                    <label className="block text-sm font-bold text-slate-700 mb-2">Project Name</label>
                                    <Input value={projectName} onChange={e => setProjectName(e.target.value)} placeholder="e.g. Cloud Migration Phase 1" className="h-12 text-lg" />
                                </div>
                            </div>
                        )}
                    </div>
            ...
            <div className="p-6 bg-white border-t border-slate-200 flex justify-between items-center z-10 shrink-0">
                 <Button variant="ghost" onClick={handleBack} disabled={currentStep === 1} size="lg" className="text-slate-500">
