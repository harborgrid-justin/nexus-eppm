...
export const WbsDictionaryPanel: React.FC<WbsDictionaryPanelProps> = ({ selectedNode, editedNode, setEditedNode, canEdit, onSave }) => {
    if (!selectedNode || !editedNode) {
        return (
            <div className="flex-1 flex flex-col items-center justify-center text-text-tertiary bg-white">
    ...
    return (
        <div className="flex-1 flex flex-col bg-white">
            ...
            <div className="flex-1 overflow-y-auto p-8 space-y-6">
                <div>
                    <label className="block text-sm font-bold text-text-primary mb-1">Element Name</label>
                    <Input value={editedNode.name} onChange={e => setEditedNode({...editedNode, name: e.target.value})} disabled={!canEdit} />
                </div>
                <div>
                    <label className="block text-sm font-bold text-text-primary mb-1">WBS Code</label>
                ...
                <div>
                    <label className="block text-sm font-bold text-text-primary mb-1">Scope Description</label>
                    <textarea 
                        className="w-full p-4 border border-border rounded-lg text-sm h-40 focus:ring-2 focus:ring-nexus-500 outline-none"
