...
import { Briefcase, Sliders, GanttChartSquare, DollarSign, AlertTriangle, Users, MessageCircle, ShoppingCart, ShieldCheck } from 'lucide-react';

interface ProjectIntegrationManagementProps {
  projectId: string;
}

const StatCard: React.FC<{ title: string; value: string | number; subtext: string; icon: React.ElementType }> = ({ title, value, subtext, icon: Icon }) => (
...
const ProjectIntegrationManagement: React.FC<ProjectIntegrationManagementProps> = ({ projectId }) => {
  ...
  if (!project || !summary || !financials || !riskProfile || !qualityProfile) {
    return <div>Loading integration data...</div>;
  }
