      ...
      <ErrorBoundary name="Integration Ledger">
        {state.integrations.length === 0 ? (
            <EmptyGrid 
                ...
                description="Initialize your enterprise fabric by bridging external ERP or BIM data sources to the Nexus ledger."
            ...
            <div className={`grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 ${theme.layout.gridGap} animate-nexus-in`}>
                {state.integrations.map((integration) => (
                    <Card key={integration.id} className="p-6 relative group flex flex-col h-full hover:border-nexus-300 transition-all">
                        <div className="flex justify-between items-start mb-4">
                            <div className={`w-12 h-12 ${theme.colors.background} rounded-xl flex items-center justify-center text-xl font-bold ${theme.colors.text.primary} border ${theme.colors.border} shadow-inner`}>
                        ...
                        <h3 className={`text-base font-black ${theme.colors.text.primary} tracking-tight`}>{String(integration.name)}</h3>
                        <p className={`text-xs ${theme.colors.text.secondary} font-bold uppercase tracking-widest mt-1`}>{String(integration.type)} Endpoint</p>
                        
                        <div className={`mt-8 flex items-center justify-between pt-4 border-t ${theme.colors.border.replace('border-','border-slate-')}50`}>
                            <span className={`text-[10px] font-mono ${theme.colors.text.tertiary} uppercase`}>Handshake: {String(integration.lastSync)}</span>
                ...
                <button 
                    ...
                    className={`border-2 border-dashed ${theme.colors.border} rounded-2xl p-6 flex flex-col items-center justify-center text-center ${theme.colors.text.tertiary} hover:border-nexus-300 hover:bg-white transition-all cursor-pointer min-h-[200px] group shadow-sm hover:shadow-md`}
                >
                    <div className={`w-12 h-12 ${theme.colors.background} rounded-full flex items-center justify-center mb-3 group-hover:scale-110 transition-transform`}>
                        <Plus size={24} className="opacity-50" />
                    </div>
                    <h3 className={`text-xs font-black uppercase tracking-widest ${theme.colors.text.secondary}`}>Provision Connector</h3>
