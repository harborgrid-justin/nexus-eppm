        ...
        <div className="space-y-10 animate-in fade-in slide-in-from-bottom-2 duration-400">
            <div className="grid grid-cols-1 gap-10">
                <div className="relative group">
                    <label className={`text-[10px] font-black ${theme.colors.text.tertiary} uppercase tracking-[0.2em] mb-3 block ml-1 flex items-center gap-2`}>
                    ...
                    <textarea 
                        ...
                        className={`w-full p-5 border-2 ${theme.colors.border} rounded-[2rem] text-sm h-48 focus:ring-8 focus:ring-nexus-500/5 focus:border-nexus-500 outline-none resize-none bg-slate-50/50 shadow-inner leading-relaxed transition-all disabled:opacity-60`}
                    />
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                    <div className={`${theme.components.card} p-8 rounded-[2.5rem] bg-white border-slate-100 shadow-sm relative overflow-hidden`}>
                        ...
                        <h3 className={`font-black ${theme.colors.text.primary} text-sm uppercase tracking-widest mb-8 flex items-center gap-3 border-b ${theme.colors.border} pb-4`}>
                            <Layers size={18} className="text-nexus-600"/> Classification Matrix
                        </h3>
                        <div className="space-y-8">
                            <div>
                                <label className={`text-[10px] ${theme.colors.text.tertiary} block mb-2 font-black uppercase tracking-wider ml-1`}>Lifecycle Category</label>
                                <select 
                                    value={co.category || 'Client Request'} 
                                    onChange={(e) => onChange('category', e.target.value)}
                                    disabled={isReadOnly}
                                    className={`w-full p-4 border ${theme.colors.border} rounded-2xl text-sm font-bold bg-white focus:ring-4 focus:ring-nexus-500/10 transition-all cursor-pointer shadow-sm ${theme.colors.text.primary} outline-none disabled:opacity-60`}
                                >
                            ...
                            <div>
                                <label className={`text-[10px] ${theme.colors.text.tertiary} block mb-3 font-black uppercase tracking-wider ml-1`}>Exposure Criticality</label>
                                    ...
                                    {['Low', 'Medium', 'High', 'Critical'].map(p => (
                                        <button
                                            ...
                                            className={`flex-1 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest border transition-all shadow-sm ${
                                                ...
                                                : `bg-white ${theme.colors.border} text-slate-400 hover:border-slate-300 hover:bg-slate-50 disabled:opacity-50`
                                            }`}
                                        >
                                            {p}
                                        </button>
                    ...
                    <div className={`${theme.components.card} p-8 rounded-[2.5rem] bg-slate-900 text-white shadow-2xl relative overflow-hidden group border border-slate-800`}>
                        ...
                        <textarea 
                            ...
                            className={`w-full p-4 bg-white/5 border-2 border-white/10 rounded-2xl text-sm h-48 resize-none focus:ring-4 focus:ring-nexus-400/20 focus:border-nexus-400 transition-all outline-none leading-relaxed relative z-10 font-medium placeholder:text-slate-600 disabled:opacity-50`}
                        />
                    </div>
                </div>
            </div>
