                ...
                <div key={stage} className={`w-80 flex-shrink-0 flex flex-col bg-slate-50 border ${theme.colors.border} rounded-3xl shadow-inner relative group/lane`}>
                    <div className={`p-5 border-b ${theme.colors.border} bg-white rounded-t-3xl flex justify-between items-center shadow-sm z-10`}>
                        ...
                        <div className="flex items-center gap-2">
                             <span className="bg-slate-100 text-slate-500 px-2 py-0.5 rounded-lg text-[10px] font-black">{(changeOrders || []).filter(co => co.status === stage).length}</span>
                    ...
                    <div className="flex-1 p-3 space-y-3 overflow-y-auto scrollbar-thin relative z-0">
                        {(changeOrders || []).filter(co => co.status === stage).map(co => (
                            <div key={co.id} className={`bg-white p-5 cursor-pointer rounded-2xl shadow-sm border border-slate-100 hover:border-nexus-300 hover:shadow-xl transition-all group active:scale-95 relative overflow-hidden`}>
                        ...
                        {(changeOrders || []).filter(co => co.status === stage).length === 0 && (
