    ...
    <div className={`${theme.components.card} overflow-hidden flex flex-col h-full`}>
        <div className={`p-3 ${theme.colors.background} border-b ${theme.colors.border} flex justify-between items-center`}>
            ...
            <div className="flex gap-2">
                <button onClick={onOpenLookup} className="text-xs flex items-center gap-1 bg-nexus-50 border border-nexus-200 px-3 py-1.5 rounded hover:bg-nexus-100 text-nexus-700 font-bold transition-colors">
                    <Book size={12}/> Lookup Item
                </button>
                <button onClick={onAddCustom} className={`text-xs flex items-center gap-1 ${theme.colors.surface} border ${theme.colors.border} px-3 py-1.5 rounded hover:${theme.colors.background} text-slate-700 font-medium transition-colors`}>
                ...
                <tbody className="divide-y divide-slate-100">
                    {estimate.items.map(item => (
                        <tr key={item.id} className={`hover:${theme.colors.background}`}>
                            ...
                            <td className="px-4 py-2">
                                <div className="flex gap-2">
                                    <input type="number" className="w-16 bg-transparent border border-slate-200 rounded px-1 text-sm text-right focus:border-nexus-500 outline-none" value={item.quantity} onChange={e => onUpdateItem(item.id, 'quantity', parseFloat(e.target.value))} />
                            ...
                            {estimate.method === 'Deterministic' ? (
                                <td className="px-4 py-2">
                                    <input type="number" className="w-full bg-transparent border border-slate-200 rounded px-1 text-sm text-right focus:border-nexus-500 outline-none" value={item.unitRate} onChange={e => onUpdateItem(item.id, 'unitRate', parseFloat(e.target.value))} />
                                </td>
                            ) : (
                                <>
