...
export const CustomBarChart: React.FC<CustomBarChartProps> = ({ 
  ...
  const effectiveBarColor = barColor || 'var(--color-info)';
  ...
  return (
    <div className={`w-full flex flex-col ${className}`} style={{ height: height }}>
      {/* Chart Area */}
      <div className="flex-1 flex items-end justify-between gap-2 relative border-b border-b-border pb-2">
        {/* Y-Axis Guidelines (Simplified) */}
        <div className="absolute inset-0 flex flex-col justify-between pointer-events-none opacity-50">
            <div className="border-t border-dashed border-border w-full h-0"></div>
            <div className="border-t border-dashed border-border w-full h-0"></div>
            <div className="border-t border-dashed border-border w-full h-0"></div>
            <div className="border-t border-dashed border-border w-full h-0"></div>
        </div>
      ...
      <div className="flex justify-between gap-2 mt-2 h-6">
        {data.map((item, index) => (
          <div key={index} className="flex-1 text-center text-[10px] text-text-secondary font-medium truncate">
            {item[xAxisKey]}
          </div>
        ))}
      </div>
