...
import { 
  Filter, Plus, RefreshCw, Layers, Loader2, Settings, ArrowLeft
} from 'lucide-react';
import { 
...
const ExtensionEngineContent: React.FC<ExtensionEngineProps> = ({ extension }) => {
  ...
  // Deterministic mock data to prevent render jitter
  const dashboardMetrics = useMemo(() => {
      const seed = extension.id.split('').reduce((acc, char) => acc + char.charCodeAt(0), 0);
      return [1, 2, 3].map(i => ({
          id: i,
          value: Math.floor(100 + ((seed * i * 37) % 900)), 
          trend: (10 + ((seed * i) % 15)).toFixed(1)
      }));
  }, [extension.id]);

  // --- SPECIALIZED SUITE ROUTING ---
  // These components provide their own full layouts
  const getSpecializedComponent = () => {
    if (extension.id === 'dod_suite') return <DoDSuite />;
    if (extension.id === 'gov_budget') return <GovBudgetSuite />;
    if (extension.id === 'construction_suite') return <ConstructionSuite />;
    if (extension.id === 'finance_suite') return <FinancialSuite />;
    if (extension.id === 'fed_gov_platform') return <FedGovSuite />;
    if (extension.id === 'state_gov_platform') return <StateGovSuite />;
    if (extension.id === 'iot_smart_site') return <IoTStream />;
    return null;
  };

  const SpecializedComponent = getSpecializedComponent();
  ...
  // --- GENERIC WRAPPER FOR STANDARD RENDERERS ---
  return (
    <PageLayout
        title={extension.name}
        subtitle={extension.description}
        icon={Layers}
        actions={<Button variant="ghost" icon={ArrowLeft} onClick={() => navigate(-1)}>Back to Marketplace</Button>}
    >
        <Suspense fallback={<div className="flex h-full items-center justify-center"><Loader2 className="animate-spin text-nexus-500"/></div>}>
            {extension.viewType === 'viewer3d' && (
                <PanelContainer className="bg-slate-900">
                    <Viewer3DRenderer extensionVersion={extension.version} />
                </PanelContainer>
            )}
            
            {extension.viewType === 'map' && (
                <PanelContainer>
                    <MapRenderer extensionName={extension.name} />
                </PanelContainer>
            )}

            {(extension.viewType === 'dashboard' || extension.viewType === 'grid' || extension.viewType === 'form') && (
                <PanelContainer className="bg-white">
                    ...
                    {(extension.viewType === 'grid' || extension.viewType === 'form') && (
                        <div className="flex-1 overflow-auto">
                            <table className="min-w-full divide-y divide-slate-200">
                                ...
                                {[...Array(10)].map((_, i) => (
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap text-slate-400"></td>
                                    </tr>
                                ))}
                                </tbody>
                            </table>
