...
export const GanttTaskList = forwardRef<HTMLDivElement, GanttTaskListProps>(({ 
  ...
  return (
    <div 
        className={`
            flex-shrink-0 border-r border-slate-200 flex flex-col bg-white z-20
                ...
                {virtualItems.map(({ index, offsetTop }) => {
                    ...
                    if (item.type === 'wbs') {
                        return (
                            <button 
                                key={item.node.id}
                                className={`group h-[44px] w-full flex items-center px-4 border-b border-slate-100 hover:bg-slate-50 text-xs text-left font-bold bg-slate-50/50 focus:outline-none transition-all flex-shrink-0 min-w-0 absolute top-0 left-0`}
                                style={{ 
                        ...
                        return (
                            <div 
                                ...
                                className={`
                                  h-[44px] w-full flex items-center px-4 border-b border-slate-100 text-xs text-left cursor-pointer transition-all flex-shrink-0 min-w-0 border-l-4 absolute top-0 left-0
