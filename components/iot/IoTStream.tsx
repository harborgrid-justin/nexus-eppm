...
export const IoTStream: React.FC = () => {
    ...
    const totalThroughput = useMemo(() => chartData.reduce((acc, curr) => acc + curr.val, 0) * 1000, [chartData]);
    const avgLatency = useMemo(() => {
        const baseLatency = 24; // Baseline ms
        const loadFactor = chartData.length > 0 ? chartData[chartData.length - 1].val / 100 : 0;
        return Math.round(baseLatency + (loadFactor * 10));
    }, [chartData]);

    return (
        <PageLayout 
            title="Field Telemetry Graph" 
            subtitle="Real-time sensor ingestion from remote sites." 
            icon={Radio}
            className="bg-slate-950 text-slate-300"
        >
            <div className="h-full p-6 font-mono overflow-hidden flex flex-col relative select-none">
                 {/* Dark Overlay/Texture handled by PageLayout className propagation */}
                 ...
                 <div className="flex justify-between items-center border-b border-green-900/50 pb-6 mb-8 relative z-10">
                     ...
                     <div className="flex gap-10 items-center">
                        <div className="text-right">
                             <p className="text-[9px] text-slate-500 uppercase font-black tracking-widest">Est. Daily Volume</p>
                             <p className="text-xl font-black text-green-400 tabular-nums">{(totalThroughput / 1000000).toFixed(1)}M <span className="text-xs font-medium">pkt</span></p>
                        </div>
                        <div className="text-right">
                             <p className="text-[9px] text-slate-500 uppercase font-black tracking-widest">Aggregate Latency</p>
                             <p className="text-xl font-black text-white tabular-nums">{avgLatency} <span className="text-xs font-medium text-slate-400">ms</span></p>
                        </div>
                     </div>
                 </div>
