    ...
    return (
        <div className="h-full bg-slate-950 p-6 text-green-500 font-mono overflow-hidden flex flex-col">
             ...
             <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 flex-1 min-h-0">
                 {/* Live Feed Chart */}
                 <div className="lg:col-span-2 bg-slate-900/50 border border-green-900/30 rounded-xl p-4 flex flex-col relative overflow-hidden">
                     <div className="flex justify-between items-center mb-4 z-10 relative">
                        <h3 className="text-sm font-bold flex items-center gap-2 text-white"><Activity size={16}/> Aggregate Signal Load</h3>
                 ...
                 <div className="bg-slate-900/50 border border-green-900/30 rounded-xl p-4 overflow-y-auto space-y-3 scrollbar-thin scrollbar-thumb-green-900">
                     ...
                     {sensors.length > 0 ? sensors.map(s => (
                         <div key={s.id} className="bg-black/40 p-3 rounded-lg border border-green-900/20 flex justify-between items-center group hover:border-green-500/50 transition-colors cursor-default">
                             <div>
                                 <div className="text-xs font-bold text-white group-hover:text-green-400 transition-colors">{s.name}</div>
                                 <div className="text-[10px] text-slate-500 mt-1 flex items-center gap-1"><MapPin size={10}/> {s.location || 'Unknown'}</div>
                             </div>
                             <div className="text-right">
                                 <div className={`text-lg font-bold ${s.maintenanceStatus === 'Good' ? 'text-green-500' : 'text-red-500'}`}>
                                     {s.maintenanceStatus === 'Good' ? 'OK' : 'ERR'}
                                 </div>
                                 <div className="text-[10px] text-slate-500 font-mono">{s.id}</div>
                             </div>
                         </div>
                     )) : (
                         <div className="text-center text-slate-500 text-xs italic p-4">No devices connected.</div>
                     )}
                 </div>
             </div>
             ...
             <div className="mt-6 grid grid-cols-1 md:grid-cols-3 gap-4">
                 {alerts.length > 0 ? alerts.map(alert => (
                     <div key={alert.id} className="bg-red-900/10 border border-red-900/50 p-3 rounded-lg flex items-center gap-3">
                         <AlertTriangle className="text-red-500 animate-pulse" size={20}/>
                         <div>
                             <p className="text-xs text-red-400 font-bold">{alert.title}</p>
                             <p className="text-[10px] text-red-300/70 truncate w-48">{alert.message}</p>
                         </div>
                     </div>
                 )) : (
                     <div className="col-span-3 bg-slate-900/30 border border-slate-800 p-3 rounded-lg flex items-center justify-center gap-3 text-slate-500 text-xs italic">
                         ...
                         System nominal. No active supply chain alerts detected in stream.
                     </div>
                 )}
             </div>
        </div>
