...
export const GlobalRiskRegister: React.FC = () => {
    ...
    const columns: Column<EnrichedRisk>[] = [
        ...
        { key: 'sourceName', header: 'Source', width: 'w-48', render: (r) => <span className={`text-xs font-bold ${theme.colors.text.secondary} truncate block max-w-[200px]`} title={r.sourceName}>{r.sourceName}</span>, sortable: true },
        { key: 'description', header: 'Risk Description', render: (r) => <span className={`text-sm font-medium ${theme.colors.text.primary} line-clamp-2`} title={r.description}>{r.description}</span> },
        ...
        { key: 'ownerId', header: 'Owner', width: 'w-32', render: (r) => <span className={`text-xs ${theme.colors.text.secondary}`}>{r.ownerId}</span> },
        { key: 'status', header: 'Status', width: 'w-24', render: (r) => <span className={`text-xs font-medium px-2 py-0.5 rounded ${theme.colors.background} border ${theme.colors.border}`}>{r.status}</span>, sortable: true }
    ];

    return (
        <div className={`h-full flex flex-col ${theme.colors.surface} rounded-xl border ${theme.colors.border} shadow-sm overflow-hidden`}>
            <div className={`p-4 border-b ${theme.colors.border} flex flex-col sm:flex-row justify-between items-center gap-4 ${theme.colors.background}`}>
                <div className="flex items-center gap-2 w-full sm:w-auto">
                    <div className="relative flex-1 sm:w-64">
                        <Search size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400"/>
                        <input 
                            className={`w-full pl-9 pr-4 py-2 text-sm border ${theme.colors.border} rounded-lg focus:ring-2 focus:ring-nexus-500 outline-none ${theme.colors.surface} ${theme.colors.text.primary}`}
                    ...
                    <select 
                        className={`p-2 text-sm border ${theme.colors.border} rounded-lg ${theme.colors.surface} focus:ring-2 focus:ring-nexus-500 outline-none cursor-pointer`}
                        value={contextFilter}
            ...
            <div className={`flex-1 overflow-hidden p-4 ${theme.colors.background}/30`}>
                 <DataTable 
                    data={filteredRisks}
                    columns={columns}
                    keyField="id"
            ...
            <div className={`p-3 border-t ${theme.colors.border} ${theme.colors.surface} text-xs ${theme.colors.text.secondary} flex justify-between px-6`}>
