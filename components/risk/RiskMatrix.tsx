    ...
    <div className={`h-full overflow-auto ${theme.layout.pagePadding}`}>
        ...
        <div className="overflow-x-auto pb-4">
            <div className="flex items-center justify-center min-w-[700px]">
                ...
                <div className={`grid grid-cols-5 grid-rows-5 gap-0 border-l border-b ${theme.colors.border}`}>
                    {scale.slice().reverse().map(prob => 
                        scale.map(imp => {
                            ...
                            return (
                                <div 
                                    key={`${prob}-${imp}`} 
                                    className={`w-32 h-32 border-r border-t ${theme.colors.border} relative p-2 flex flex-wrap gap-1 content-start ${getCellColor(prob, imp)}`}
                                ...
                                >
                                    ...
                                    {cellRisks.length > 5 && <span className="text-xs text-slate-500 font-bold pl-1">+{cellRisks.length - 5}</span>}
                                </div>
                            );
                        })
                    )}
