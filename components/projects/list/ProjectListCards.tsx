  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 p-4 pb-20">
      {projects.map(project => {
        ...
        return (
          <div key={project.id} onClick={() => navigate(`/projectWorkspace/${project.id}`)} className={`${theme.components.card} p-4 hover:shadow-md active:scale-[0.98] transition-all cursor-pointer flex flex-col min-w-0 h-full`}>
            <div className="flex justify-between items-start mb-4 gap-2">
              <div className="min-w-0 flex-1">
                <h3 className="text-sm font-bold text-slate-900 leading-tight truncate">{project.name}</h3>
                <p className="text-[10px] font-mono text-slate-500 mt-1 truncate">{project.code}</p>
              </div>
              <StatusBadge status={project.health} variant="health" className="flex-shrink-0 scale-90 origin-top-right"/>
            </div>
            
            <div className="flex justify-between items-center text-[11px] text-slate-600 mb-6 gap-4">
      ...
      {projects.length === 0 && <div className="col-span-full text-center py-12 text-slate-400">No projects found.</div>}
    </div>
  );
};
