...
import { CheckCircle, Loader2, XCircle, AlertCircle, ChevronDown, ChevronRight, FileText, AlertTriangle } from 'lucide-react';
                                                ...
                                                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                                    <div className="space-y-4">
                                                        ...
                                                        <div className="bg-white p-4 rounded-lg border border-slate-200 space-y-2 text-sm text-slate-600">
                                                            <p><strong className="text-slate-800">File Name:</strong> {job.fileName || 'N/A'}</p>
                                                            <p><strong className="text-slate-800">Size:</strong> {job.fileSize || 'N/A'}</p>
                                                            <p><strong className="text-slate-800">Message:</strong> {job.details}</p>
                                                            <p className="flex items-center gap-2 mt-2">
                                                                <strong className="text-slate-800">Orphans Processed:</strong> 
                                                                <span className="px-2 py-0.5 bg-yellow-100 text-yellow-800 rounded text-xs font-bold">0 Records</span>
                                                            </p>
                                                        </div>
                                                    </div>
                                                    <div className="space-y-4">
                                                        ...
                                                        <div className="bg-slate-900 p-4 rounded-lg border border-slate-700 text-xs font-mono text-green-400 overflow-y-auto max-h-32">
                                                            <p>[INFO] Job initialized by {job.submittedBy}</p>
                                                            <p>[INFO] Validating schema against v2.4 spec...</p>
                                                            <p>[INFO] Schema validation passed.</p>
                                                            {job.status === 'Completed' ? (
                                                                <p>[SUCCESS] Committed 142 records to database.</p>
                                                            ) : job.status === 'Failed' ? (
                                                                <p className="text-red-400">[ERROR] Connection timeout at row 452. Transaction rolled back.</p>
                                                            ) : (
                                                                <p>[INFO] Processing batch 1 of 4...</p>
                                                            )}
                                                        </div>
                                                    </div>
                                                </div>
