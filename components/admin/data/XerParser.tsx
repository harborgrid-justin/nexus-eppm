...
export const XerParser: React.FC = () => {
    ...
    return (
        <div className={`h-full flex flex-col ${theme.colors.surface} overflow-hidden shadow-inner`}>
            <div className={`p-10 border-b border-slate-100 flex justify-between items-center ${theme.colors.background}/30`}>
                <div>
                    <h2 className={`text-2xl font-black ${theme.colors.text.primary} flex items-center gap-4 uppercase tracking-tighter`}>
                        <FileCode className="text-nexus-600" size={32} /> Schedule Binary Ingestion
                    </h2>
                    <p className={`text-sm ${theme.colors.text.secondary} mt-1 font-medium`}>Directly import .XER (Primavera) or .MPP (MS Project) artifacts.</p>
                </div>
            ...
            <div className="flex-1 p-12 overflow-y-auto scrollbar-thin">
                {status === 'idle' && (
                    <div className={`max-w-2xl mx-auto text-center border-4 border-dashed ${theme.colors.border} rounded-[3rem] p-20 ${theme.colors.background} hover:${theme.colors.surface} hover:border-nexus-400 transition-all group cursor-pointer relative shadow-inner nexus-empty-pattern`}>
                        <input type="file" accept=".xer,.mpp,.xml" className="absolute inset-0 opacity-0 cursor-pointer z-10" onChange={handleFileUpload} />
                        <div className={`w-24 h-24 ${theme.colors.surface} rounded-3xl shadow-2xl flex items-center justify-center mx-auto mb-10 group-hover:scale-110 transition-transform rotate-3 group-hover:rotate-0 duration-500 border ${theme.colors.border}`}>
                            <Upload size={48} className="text-nexus-500" />
                        </div>
                        <h3 className={`text-xl font-black ${theme.colors.text.primary} mb-2 uppercase tracking-tighter`}>
                            {file ? file.name : "Drop Schedule Artifact Here"}
                        </h3>
                        <p className={`text-sm ${theme.colors.text.tertiary} font-medium`}>
                    ...
                    <div className="max-w-5xl mx-auto animate-nexus-in">
                        ...
                        <div className="grid grid-cols-2 lg:grid-cols-4 gap-6 mb-12">
                            {[
                                ...
                                <div key={idx} className={`p-8 ${theme.colors.surface} border ${theme.colors.border} rounded-[2rem] shadow-sm text-center group hover:border-nexus-300 transition-all`}>
                                    <div className={`${s.color} mb-4 opacity-40 group-hover:opacity-100 transition-opacity`}><s.icon className="mx-auto" size={28}/></div>
                                    <div className={`text-4xl font-black ${theme.colors.text.primary} font-mono tracking-tighter`}>{s.val}</div>
                                    <div className={`text-[10px] font-black ${theme.colors.text.tertiary} uppercase tracking-widest mt-2`}>{s.label}</div>
                                </div>
                            ))}
                        </div>

                        <div className={`flex justify-end gap-4 mt-12 pb-20 border-t ${theme.colors.border} pt-8`}>
