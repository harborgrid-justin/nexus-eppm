...
const StrategicAlignmentBoard: React.FC = () => {
    ...
    return (
        <div className={`h-full overflow-hidden flex flex-col ${theme.layout.pagePadding} ${theme.colors.background} animate-in fade-in duration-500`}>
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-10 border-b border-slate-100 pb-8">
                <div className="flex items-center gap-4">
                    <div className="p-4 bg-slate-900 text-white rounded-2xl shadow-xl shadow-slate-900/20"><Target size={28}/></div>
                    <div>
                        <h2 className={`text-2xl font-black text-slate-900 uppercase tracking-tighter`}>Strategic Alignment Workbench</h2>
                        <p className={`text-sm ${theme.colors.text.secondary} font-medium mt-1`}>Visualizing investment distribution across organizational strategic pillars.</p>
                    </div>
                </div>
                <div className="flex items-center gap-10">
                    <div className="text-right">
                        <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Portfolio Capital</p>
                        <p className="text-2xl font-black text-slate-900 font-mono">{formatCompactCurrency(totalPortfolioBudget)}</p>
                    </div>
                    <div className={`h-10 w-px bg-slate-200 hidden md:block`}></div>
                    <div className="flex items-center gap-3 bg-white border border-slate-200 px-5 py-3 rounded-2xl shadow-sm">
                        <ShieldCheck size={20} className="text-nexus-600"/>
                        ...
                        <div key={column.id} className={`w-[360px] flex flex-col h-full bg-slate-50 border ${theme.colors.border} rounded-3xl shadow-inner relative overflow-hidden group/lane`}>
                            ...
                            <div className={`p-6 border-b ${theme.colors.border} rounded-t-3xl bg-white shadow-sm z-10`}>
                                ...
                                <div className={`w-full bg-slate-100 h-2 rounded-full mt-4 overflow-hidden border border-slate-200 shadow-inner`}>
                                    <div className="h-full bg-nexus-600 transition-all duration-1000 shadow-[0_0_8px_rgba(14,165,233,0.3)]" style={{ width: `${Math.min(100, (column.totalBudget / 20000000) * 100)}%` }}></div>
                                </div>
                            </div>
                                ...
                                {column.projects.length > 0 ? (
                                    ...
                                    <div className="h-full py-8">
                                         <div className="h-full flex flex-col items-center justify-center nexus-empty-pattern rounded-3xl border-2 border-dashed border-slate-200">
                                             ...
                                             <button className="mt-4 text-[10px] font-black text-nexus-600 uppercase hover:underline">Draft Proposition</button>
                                         </div>
                                    </div>
                                )}
                            </div>
