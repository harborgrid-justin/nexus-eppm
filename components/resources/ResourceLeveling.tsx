...
const ResourceLeveling: React.FC<ResourceLevelingProps> = ({ overAllocatedResources }) => {
    ...
    // Strategy: Derive 6-week horizon labels from current data date
    ...
    // Production Algorithm: Aggregates real demand from all projects in the ledger
            ...
            state.projects.forEach(p => {
                p.tasks.forEach(t => {
                    ...
                    t.assignments.filter(a => overAllocatedResources.some(r => r.id === a.resourceId)).forEach(assign => {
                        // Effort Calculation: (Units/100) * 40h standard week
    ...
    return (
        <div className="h-full flex flex-col animate-in fade-in duration-300">
            <div className="p-4 border-b border-slate-200 flex-shrink-0 flex items-center justify-between bg-slate-50/50">
                <div>
                    <h3 className="font-bold text-slate-700 text-sm">Resource Alignment Engine</h3>
                    <p className="text-[10px] text-slate-400 uppercase font-black tracking-widest">Cross-Portfolio Capacity Balancing</p>
                </div>
                <button 
                    onClick={runLevelingSimulation}
                    disabled={isLeveling || !overAllocatedResources?.length}
                    className="px-4 py-2 bg-nexus-600 rounded-xl text-xs font-black uppercase tracking-widest text-white hover:bg-nexus-700 flex items-center gap-2 disabled:opacity-50 shadow-lg shadow-nexus-500/20 transition-all active:scale-95"
                >
                    {isLeveling ? <RefreshCw className="animate-spin" size={14}/> : <Sliders size={14}/>}
                    {isLeveling ? 'Resolving...' : 'Level Resources'}
                </button>
            </div>
                ...
                {simulatedData ? (
                    <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm h-80 animate-nexus-in">
                        <div className="flex justify-between items-center mb-6">
                            <h4 className="font-bold text-slate-800 text-sm uppercase tracking-tight">Projected Demand Curve (Post-Resolution)</h4>
                            <div className="flex gap-4 text-[10px] font-black uppercase tracking-tighter">
                                <span className="flex items-center gap-1.5"><div className="w-2 h-2 bg-red-500 rounded-full"></div> Unresolved</span>
                                <span className="flex items-center gap-1.5"><div className="w-2 h-2 bg-green-500 rounded-full"></div> Leveled</span>
                            </div>
                        </div>
                        <ResponsiveContainer width="100%" height="100%">
                            <BarChart data={simulatedData}>
                                <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#f1f5f9" />
                                <XAxis dataKey="period" tick={{fontSize: 9, fontWeight: 'bold', fill: '#94a3b8'}} />
                                <YAxis tick={{fontSize: 9, fontWeight: 'bold', fill: '#94a3b8'}} />
                                <Tooltip cursor={{fill: '#f8fafc'}} contentStyle={{borderRadius: '12px', border: 'none', boxShadow: '0 10px 15px -3px rgba(0,0,0,0.1)'}} />
                                <Bar dataKey="Before" fill="#ef4444" name="Unleveled" radius={[4,4,0,0]} />
                                <Bar dataKey="After" fill="#22c55e" name="Leveled" radius={[4,4,0,0]} />
                                <ReferenceLine y={simulatedData[0]?.Capacity} stroke="#f97316" strokeDasharray="5 5" label={{ value: 'MAX CAPACITY', position: 'insideTopRight', fontSize: 9, fill: '#f97316', fontWeight: 'bold' }} />
                            </BarChart>
                        </ResponsiveContainer>
                    </div>
                ) : (
                    <div className="h-80 flex flex-col">
                        <EmptyGrid 
                            ...
                            description="The resource leveling solver uses Heuristic Priority rules (Total Float) to resolve over-allocations across the EPS."
                            icon={BarChart2}
                            actionLabel="Run Capacity Simulation"
                        ...
                        <div key={res.id} className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm group hover:border-nexus-300 transition-all">
                            <div className="flex justify-between items-start mb-4">
                                ...
                                <div className="text-right">
                                    ...
                                    <p className="text-[9px] text-slate-400 font-bold uppercase mt-1">Variance: +{Math.round(res.allocated - (res.capacity || 160))}h</p>
                                </div>
                            </div>
                            
                            {simulatedData ? (
                                <div className="p-2.5 bg-green-50 text-green-800 text-[11px] rounded-lg border border-green-200 flex items-center gap-2 animate-nexus-in font-medium">
                                    <Check size={14} className="shrink-0"/> 
                                    <span>Resolution: Solver proposes 3-day shift of non-critical assignments.</span>
                                </div>
                            ) : (
