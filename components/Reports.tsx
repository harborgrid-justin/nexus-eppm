  ...
  return (
    <div className={`${theme.layout.pageContainer} ${theme.layout.pagePadding} ${theme.layout.sectionSpacing}`}>
      ...
      <div className={theme.layout.panelContainer}>
          <div className={`flex-1 grid grid-cols-1 lg:grid-cols-[350px_1fr] gap-0 lg:divide-x ${theme.colors.border} overflow-hidden bg-white`}>
                ...
                <div className="flex-1 overflow-y-auto p-4 space-y-6 scrollbar-thin">
                    <div>
                        ...
                        <select
                            ...
                            className={`w-full p-2.5 bg-white border ${theme.colors.border} rounded-lg text-sm font-bold ${theme.colors.text.primary} outline-none focus:ring-2 focus:ring-nexus-500`}
                        >
                    ...
                    <div>
                        ...
                        <div className={`space-y-1 max-h-60 overflow-y-auto border ${theme.colors.border} rounded-lg p-2 bg-white`}>
                            {availableColumns.map(col => (
                            <label key={col.id} className={`flex items-center gap-3 p-2 rounded-md hover:bg-slate-50 text-sm font-medium ${theme.colors.text.secondary} cursor-pointer transition-colors`}>
                                <input 
                                    type="checkbox" 
                    ...
                    <div>
                        ...
                        <button className={`w-full p-3 text-xs font-bold border-2 border-dashed ${theme.colors.border} rounded-xl ${theme.colors.text.tertiary} hover:border-nexus-300 hover:text-nexus-600 transition-all bg-white`}>
                            + Define Filter Criteria
                        </button>
                    </div>
                </div>
                <div className={`p-4 border-t ${theme.colors.border} bg-white flex gap-2`}>
                    <Button 
