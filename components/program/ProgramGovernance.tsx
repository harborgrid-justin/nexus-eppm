...
const ProgramGovernance: React.FC<ProgramGovernanceProps> = ({ programId }) => {
  ...
  const RoleCard = ({ role, colorClass }: { role: any, colorClass: string }) => {
      if (!role) return null;
      return (
        <div className={`p-4 rounded-xl border shadow-sm text-center w-64 flex-shrink-0 ${colorClass}`}>
            ...
            <div className="w-full overflow-x-auto pb-4">
              <div className="flex flex-col items-center min-w-[800px]">
                ...
                <div className="mt-8 flex flex-col items-center w-full max-w-3xl">
                    ...
                    <div className="flex justify-between w-2/3 mt-4 gap-4">
                        <div className="px-4 py-2 bg-slate-50 border border-slate-200 rounded text-xs text-slate-500 font-medium whitespace-nowrap">Project Manager A</div>
                        <div className="px-4 py-2 bg-slate-50 border border-slate-200 rounded text-xs text-slate-500 font-medium whitespace-nowrap">Project Manager B</div>
                    </div>
                </div>
              </div>
            </div>
