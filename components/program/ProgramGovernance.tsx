...
const ProgramGovernance: React.FC<ProgramGovernanceProps> = ({ programId }) => {
  ...
  const RoleCard = ({ role, isPrimary }: { role: GovernanceRole | undefined, isPrimary?: boolean }) => {
      if (!role) return <div className={`w-64 h-32 border border-dashed ${theme.colors.border} rounded-xl flex items-center justify-center ${theme.colors.text.tertiary} text-[10px] font-black uppercase tracking-widest bg-slate-50/50`}>Role Not Assigned</div>;
      ...
      return (
        <div className={`p-4 rounded-xl border shadow-sm text-center w-64 flex-shrink-0 ${cardClasses} ${isPrimary ? 'border-nexus-700 shadow-xl' : theme.colors.border} relative group`}>
            ...
            <div className={`font-bold text-[10px] uppercase tracking-widest mb-1 ${isPrimary ? 'opacity-80' : theme.colors.text.secondary}`}>{role.role}</div>
            <div className={`font-black text-lg mb-1 uppercase tracking-tight`}>{role.assigneeId}</div>
            <div className={`text-[10px] ${isPrimary ? 'opacity-70' : theme.colors.text.secondary} px-2 leading-relaxed font-medium`}>{role.responsibilities}</div>
        </div>
      );
  };
  ...
  return (
    <div className={`h-full overflow-y-auto ${theme.layout.pagePadding} ${theme.layout.sectionSpacing} animate-in fade-in duration-300 scrollbar-thin`}>
        ...
        <p className={`${theme.colors.text.secondary} text-sm mb-6 font-medium`}>Established decision-making authority, escalation paths, and approval gates.</p>
        ...
        <div className={`${theme.colors.surface} p-8 rounded-xl border ${theme.colors.border} shadow-sm flex flex-col items-center relative overflow-hidden min-h-[400px] justify-center`}>
            {hasCoreRoles ? (
                <>
                    <h3 className="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-8 absolute left-6 top-6 flex items-center gap-2">
                            ...
                            <div className="grid grid-cols-3 gap-4">
                                    ...
                                    <div key={role.id} className={`p-4 ${theme.colors.background} border ${theme.colors.border} rounded-xl relative group shadow-sm`}>
                                        <div className="flex justify-between items-start">
                                            <div>
                                                ...
                                                <p className="text-[10px] font-bold text-slate-500 mt-0.5">{role.assigneeId}</p>
                                            </div>
                                ...
                                {governanceRoles.filter(r => !['Sponsor', 'Steering Committee', 'Program Manager'].includes(r.role)).length === 0 && (
                                    <div className="col-span-3">
                                        <EmptyGrid 
                                            title="No Extended Roles" 
                                            description="Provision technical or legal advisors to the governance board."
                                            icon={Users}
                                        />
                                    </div>
                                )}
                ...
                <EmptyGrid 
                    ...
                    icon={Gavel}
                    actionLabel="Assign Core Leadership"
            ...
            {governanceEvents.length > 0 ? (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {governanceEvents.map(event => (
                        <div key={event.id} className={`flex items-center justify-between p-4 ${theme.colors.background} border ${theme.colors.border} rounded-xl hover:border-nexus-300 transition-colors shadow-sm`}>
                            <div className="flex items-center gap-3">
                                <div className={`p-3 ${theme.colors.surface} rounded-xl shadow-inner border ${theme.colors.border} ${theme.colors.text.secondary}`}>
                                    <Clock size={20}/>
                                </div>
                                <div>
                                    <p className="text-sm font-black text-slate-800 uppercase tracking-tight">{event.name}</p>
                                    <p className="text-[10px] text-slate-400 uppercase tracking-wide mt-0.5 font-bold">{event.type}</p>
                                </div>
                            </div>
                            <div className="text-right">
                                <p className="text-sm font-mono font-black text-nexus-700">{event.nextDate}</p>
                                <span className="text-[9px] uppercase font-black text-slate-400 tracking-tighter">{event.frequency}</span>
                            </div>
                        </div>
                    ))}
                </div>
                ...
                <div className="p-4">
                    <EmptyGrid 
                        title="Review Cadence Inactive"
                        description="No recurring governance events or steering committee syncs have been scheduled."
                        icon={Calendar}
                    />
                </div>
            )}
        ...
        <SidePanel
            ...
            title="Provision Governance Role"
            width="md:w-[500px]"
            footer={
                <>
        ...
        >
            <div className="space-y-6">
                <div>
                    <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1.5 ml-1">Role Title</label>
                    <Input value={newRole.role} onChange={e => setNewRole({...newRole, role: e.target.value})} placeholder="e.g. Chief Architect" />
                </div>
                <div>
                    <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1.5 ml-1">Assignee ID</label>
                ...
                <div>
                    ...
                    <select 
                        className={`w-full p-2.5 border ${theme.colors.border} rounded-lg text-sm bg-slate-50 font-bold focus:ring-2 focus:ring-nexus-500 outline-none`}
                ...
                <div>
                    <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1.5 ml-1">Responsibilities</label>
                    <textarea 
                        className="w-full p-3 border border-slate-300 rounded-lg text-sm h-32 focus:ring-2 focus:ring-nexus-500 outline-none resize-none bg-slate-50 font-medium"
