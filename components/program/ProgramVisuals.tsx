    ...
    return (
        <div className={`grid grid-cols-1 lg:grid-cols-2 ${theme.layout.gridGap}`}>
            <Card className={`${theme.layout.cardPadding} h-[400px] flex flex-col`}>
                <h3 className={`font-bold ${theme.colors.text.primary} mb-6`}>Financial Comparison</h3>
            ...
            <Card className={`${theme.layout.cardPadding} h-[400px] flex flex-col`}>
                <h3 className={`font-bold ${theme.colors.text.primary} mb-6`}>Execution Summary</h3>
                <div className="flex-1 flex flex-col justify-start space-y-4 overflow-y-auto pr-2">
                    {sortedProjects.map(p => (
                        <div key={p.id}>
                            <div className="flex justify-between text-xs mb-1">
                                <span className={`font-bold ${theme.colors.text.secondary}`}>{p.name}</span>
                            ...
                            <div className={`flex justify-between text-[10px] ${theme.colors.text.tertiary} mt-1 font-mono`}>
                                <span>{p.startDate}</span>
                                <span>{p.endDate}</span>
                            </div>
                        </div>
                    ))}
                    {sortedProjects.length === 0 && <div className={`text-center ${theme.colors.text.tertiary} text-sm italic py-10`}>No projects linked to this program.</div>}
                </div>
            </Card>
        </div>
    );
