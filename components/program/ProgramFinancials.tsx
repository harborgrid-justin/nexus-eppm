  ...
  return (
    <div className={`h-full overflow-y-auto ${theme.layout.pagePadding} space-y-8 animate-in fade-in duration-300 scrollbar-thin`}>
        ...
        <Card className={`${theme.layout.cardPadding} h-[400px] flex flex-col shadow-sm`}>
            <div className={`flex justify-between items-center mb-8 border-b ${theme.colors.border} pb-4`}>
                <h3 className={`font-black text-[10px] uppercase tracking-widest ${theme.colors.text.tertiary}`}>Budget Deployment vs. Linear Forecast</h3>
        ...
        <div className={`${theme.colors.surface} rounded-[2.5rem] border ${theme.colors.border} shadow-sm overflow-hidden flex flex-col`}>
            <div className={`p-6 border-b ${theme.colors.border} ${theme.colors.background}/50 flex justify-between items-center`}>
            ...
            {programFinancials.gates.length > 0 ? (
                <div className={`grid grid-cols-1 md:grid-cols-3 divide-y md:divide-y-0 md:divide-x ${theme.colors.border.replace('border-', 'divide-')}`}>
                    {programFinancials.gates.map(gate => (
                        <div key={gate.id} className={`p-8 flex flex-col items-center text-center relative group hover:${theme.colors.background}/50 transition-colors`}>
                            ...
                            <h4 className={`font-black ${theme.colors.text.primary} text-sm uppercase tracking-tight`}>{gate.name}</h4>
                            <p className={`text-[10px] ${theme.colors.text.tertiary} font-bold uppercase mt-1`}>{gate.milestoneTrigger}</p>
                            <p className={`text-2xl font-black ${theme.colors.text.primary} mt-4 font-mono`}>{formatCompactCurrency(gate.amount)}</p>
        ...
        >
            <div className="space-y-4">
                <p className={`text-xs font-bold ${theme.colors.text.tertiary} uppercase tracking-widest bg-slate-50 p-3 rounded-lg border mb-6`}>
                ...
                {editAllocations.map(alloc => (
                    <div key={alloc.id} className={`p-5 bg-white rounded-2xl border ${theme.colors.border} shadow-sm group hover:border-nexus-300 transition-all`}>
                         <div className="flex justify-between items-center mb-3">
                            <span className={`text-sm font-black ${theme.colors.text.primary} uppercase tracking-tight`}>
                         ...
                         <div className="relative">
                             ...
                             <input 
                                ...
                                className={`w-full pl-8 pr-4 py-3 text-lg font-mono font-black border ${theme.colors.border} rounded-xl focus:ring-4 focus:ring-nexus-500/10 focus:border-nexus-500 outline-none transition-all`}
                             />
                         </div>
                    </div>
                ))}
