...
import { Map, Link as LinkIcon, AlertTriangle, Calendar, Flag } from 'lucide-react';
    ...
    <div className={`h-full overflow-y-auto ${theme.layout.pagePadding} space-y-6 animate-in fade-in duration-300`}>
        <div className="flex items-center justify-between mb-2">
            <div className="flex items-center gap-2">
                <Map className="text-nexus-600" size={24}/>
            ...
            <table className="min-w-full divide-y divide-slate-200">
                ...
                <tbody className="bg-white divide-y divide-slate-100">
                    {programDependencies.map(dep => {
                        ...
                        return (
                            <tr key={dep.id}>
                                <td className="px-6 py-3 text-sm font-medium text-slate-900">{source?.code || dep.sourceProjectId}</td>
                                <td className="px-6 py-3 text-sm font-medium text-slate-900">{target?.code || dep.targetProjectId}</td>
                                <td className="px-6 py-3 text-sm text-slate-600">{dep.type}</td>
                                <td className="px-6 py-3 text-sm text-slate-600">{dep.description}</td>
                                <td className="px-6 py-3">
                                    {dep.status === 'Critical' ? (
                                        ...
                                        <span className="text-xs font-bold text-slate-600 bg-slate-100 px-2 py-1 rounded-full">{dep.status}</span>
                                    )}
                                </td>
                            </tr>
                        );
                    ...
                    {programDependencies.length === 0 && (
                        <tr><td colSpan={5} className="px-6 py-8 text-center text-slate-500 italic">No critical cross-project dependencies identified.</td></tr>
                    )}
                </tbody>
            </table>
        </div>
