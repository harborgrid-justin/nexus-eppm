...
export const rootReducer = (state: DataState, action: Action): DataState => {
  ...
  } else if (action.type.startsWith('SYSTEM_') || action.type.startsWith('GOVERNANCE_') || action.type === 'UPDATE_USER' || action.type === 'MARK_ALERT_READ') {
      nextState = systemReducer(state, action as any);
  } else if (action.type.includes('VENDOR') || action.type.includes('CONTRACT') || action.type.includes('PURCHASE_ORDER')) {
  ...
  } else if (action.type.includes('BUDGET_ITEM') || action.type.includes('EXPENSE') || action.type === 'APPROVE_CHANGE_ORDER' || action.type === 'TRANSFER_BUDGET') {
      nextState = financialReducer(state, action as any);
  } else if (action.type.startsWith('FIELD_') || action.type.startsWith('ROADMAP_') || action.type === 'ADD_ACTIVITY') {
